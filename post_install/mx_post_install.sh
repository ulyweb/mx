...
check_systemd() {
    if ! pidof systemd &>/dev/null && [ "$(cat /proc/1/comm 2>/dev/null)" != "systemd" ]; then
        ...
        echo "  At MX Linux install, choose systemd as the init system."
        echo "============================================"
        exit 1
    fi
    echo "[OK] systemd detected — proceeding."
}

enable_service() {
    local svc="$1"
    echo "[+] Enabling and starting service: $svc"
    sudo systemctl enable --now "$svc" || echo "[WARN] Could not enable $svc — may not be installed yet."
}
...
configure_firewall_samba() {
    ...
    # Enable UFW as a systemd service so it persists across reboots
    enable_service ufw
...
install_virtualization() {
    ...
    enable_service libvirt-guests   # Saves/restores VM state on reboot
...
install_nodejs() {
    ...
    # Generate systemd startup script for PM2 so Node apps survive reboots
    echo "[+] Configuring PM2 systemd startup..."
    pm2 startup systemd -u "$USER" --hp "$HOME" 2>/dev/null | grep "sudo" | bash || \
...
install_timeshift() {
    ...
    # Timeshift is in MX/Debian repos
    sudo apt update
...
update_firmware() {
    ...
    # Enable fwupd daemon under systemd
...
install_docker() {
    ...
    # Enable Docker daemon under systemd
